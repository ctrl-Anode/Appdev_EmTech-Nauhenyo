<template>
  <div class="admin-panel">
    <h2>Admin Panel - Manage Incident Reports</h2>
...
export default {
  ...
  computed: {
    ...
    return this.reports
      .filter((report) => {
        ...
        const categoryMatch = report.category ? report.category.toLowerCase().includes(this.searchQuery.toLowerCase()) : false;
        const subCategoryMatch = report.subCategory ? report.subCategory.toLowerCase().includes(this.searchQuery.toLowerCase()) : false;
        const locationMatch = report.location ? report.location.toLowerCase().includes(this.searchQuery.toLowerCase()) : false;
        const statusMatch = report.status ? report.status.toLowerCase().includes(this.searchQuery.toLowerCase()) : false;
      ...
      .sort((a, b) => {
        ...
        return 0; // No sorting if sortByStatus is false
      });
  },
  ...
  methods: {
    ...
    async updateReport() {
      try {
        const response = await fetch(`http://localhost:3000/api/incidents/${this.selectedReport.id}`, {
          method: "PUT",
